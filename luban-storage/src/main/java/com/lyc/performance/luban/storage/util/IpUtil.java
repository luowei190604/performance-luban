...
public class IpUtil {
    ...
    public static String perferAddress(String perferStr) throws SocketException {
        List<Inet4Address> addresss = getLocalIp4AddressFromNetworkInterface();
        for (InetAddress adress : addresss) {
            String hostAddress = adress.getHostAddress();
            if (hostAddress.contains(perferStr)) {
                return hostAddress;
            }
        }
        return addresss.size() != 0 ? addresss.get(0).getHostAddress() : "";
    }
